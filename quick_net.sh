...
parabin() {
  $trgdir/parachain-template-node "$@"
}
polkabin() {
  $trgdir/polkadot "$@"
}
#./patch.sh
cp $trgdir/parachain-template-node $trgdir/polkadot
#cp $parabin $polkabin
#tmp=`mktemp -d`
tmp=/tmp/mytmp
tmp2=/tmp/mytmp2
...
#export RUST_LOG="parachain::pvf=debug"
...
nix-shell -p yarn --run 'yarn exec node' <<EOF
const fs = require('fs');
...
echo "BOOTRELAY1 /ip4/$ip/tcp/9401/ws/p2p/$relay01nk"
echo "BOOTRELAY2 /ip4/$ip/tcp/9402/ws/p2p/$relay02nk"
echo "BOOTRELAY3 /ip4/$ip/tcp/9403/ws/p2p/$relay03nk"
echo "BOOTCOL1 /ip4/$ip/tcp/9421/ws/p2p/$col01nk"
echo "BOOTCOL2 /ip4/$ip/tcp/9422/ws/p2p/$col02nk"
#exit
...
polkabin --unsafe-ws-external --unsafe-rpc-external --no-prometheus --no-telemetry --rpc-cors all --rpc-methods Unsafe \
          ...
          --listen-addr "/ip4/$ip/tcp/9301" \
          --listen-addr "/ip4/$ip/tcp/9401/ws" \
          --bootnodes /ip4/$ip/tcp/9302/p2p/$relay02nk \
          --base-path $tmp/rel01 \
...
polkabin --unsafe-ws-external --unsafe-rpc-external --no-prometheus --no-telemetry --rpc-cors all --rpc-methods Unsafe \
          ...
          --listen-addr "/ip4/$ip/tcp/9302" \
          --listen-addr "/ip4/$ip/tcp/9402/ws" \
          --bootnodes /ip4/$ip/tcp/9301/p2p/$relay01nk \
          --base-path $tmp/rel02 \
...
polkabin --unsafe-ws-external --unsafe-rpc-external --no-prometheus --no-telemetry --rpc-cors all --rpc-methods Unsafe \
          ...
          --listen-addr "/ip4/$ip/tcp/9303/ws" \
          --listen-addr "/ip4/$ip/tcp/9403/ws" \
          --bootnodes /ip4/$ip/tcp/9301/p2p/$relay01nk \
          --bootnodes /ip4/$ip/tcp/9302/p2p/$relay02nk \
          --base-path $tmp/rel03 \
...
parabin --collator --unsafe-ws-external --unsafe-rpc-external --no-prometheus --no-telemetry --rpc-cors all --rpc-methods Unsafe \
         ...
         --listen-addr "/ip4/$ip/tcp/9321" \
         --listen-addr "/ip4/$ip/tcp/9421/ws" \
         --alice --chain $tmp/para_raw.json \
         --base-path $tmp/col01 \
         --bootnodes /ip4/$ip/tcp/9322/p2p/$col02nk \
         -- \
         --bootnodes /ip4/$ip/tcp/9301/p2p/$relay01nk \
         --bootnodes /ip4/$ip/tcp/9302/p2p/$relay02nk \
...
parabin --collator --unsafe-ws-external --unsafe-rpc-external --no-prometheus --no-telemetry --rpc-cors all --rpc-methods Unsafe \
         ...
         --listen-addr "/ip4/$ip/tcp/9322" \
         --listen-addr "/ip4/$ip/tcp/9422/ws" \
         --bob --chain $tmp/para_raw.json \
         --base-path $tmp/col02 \
         --bootnodes /ip4/$ip/tcp/9321/p2p/$col01nk \
         -- \
         --bootnodes /ip4/$ip/tcp/9301/p2p/$relay01nk \
         --bootnodes /ip4/$ip/tcp/9302/p2p/$relay02nk \
